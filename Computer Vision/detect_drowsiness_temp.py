		...
		for rect in rects:
			...
			# if leftEAR < EYE_AR_THRESH or rightEAR < EYE_AR_THRESH  :
				...
				# if leftEAR < EYE_AR_THRESH  :
					# (2000+endtime-int(round(time.time() * 1000)))/2000 
			weight=(endtime+float(int(round(time.time() * 1000)))) / endtime
			leftEARAverage+=weight*leftEAR
			leftWeights+=weight
			# if rightEAR < EYE_AR_THRESH :
			rightEARAverage+=weight*rightEAR
			rightWeights+=weight
				# if the eyes were closed for a sufficient number of
				# then sound the alarm
				# if COUNTER >= EYE_AR_CONSEC_FRAMES and abs(leftCounter-rightCounter)  < FRAME_DIFFERENCE_THRESH:
			...
			cv2.putText(frame, "L-EAR: {:.2f}".format(leftEAR), (300, 30),
				cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
			cv2.putText(frame, "R-EAR: {:.2f}".format(rightEAR), (450, 30),
				cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
